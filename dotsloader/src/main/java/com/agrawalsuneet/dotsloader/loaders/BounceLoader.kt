...
class BounceLoader : LinearLayout, LoaderContract {
    ...
    private fun getBallAnimation(): Animation {
        return when (state) {
            ...
            STATE_SQUEEZING -> {
                ScaleAnimation(1.0f, 1.0f, 1.0f, 0.9f
            ...
            STATE_RESIZING -> {
                ScaleAnimation(1.0f, 1.0f, 0.9f, 1.0f
                        , ballRadius.toFloat(), (2 * ballRadius).toFloat())
                        .apply {
                            duration = (animDuration / 10).toLong()
    ...
    private fun getShadowAnimation(): AnimationSet {
        ...
        when (state) {
            STATE_COMINGUP -> {
                transAnim = TranslateAnimation(0.0f, (-6 * ballRadius).toFloat(),
                ...
                transAnim = TranslateAnimation((-6 * ballRadius).toFloat(), 0.0f,
